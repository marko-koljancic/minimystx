                      ...
                      .then((resolvedResult: any) => {
                        set((draft) => {
                          if (draft.subFlows[context.geoNodeId]?.nodeRuntime[node.id]) {
                            draft.subFlows[context.geoNodeId].nodeRuntime[node.id].output =
                              resolvedResult;
                          }
                        });
                      })
                      ...
                      result.then((resolvedResult: any) => {
                        set((draft) => {
                          if (draft.subFlows[context.geoNodeId]?.nodeRuntime[nodeId]) {
                            draft.subFlows[context.geoNodeId].nodeRuntime[nodeId].output =
                              resolvedResult;
                          }
                        });
                      });
                        ...
                        result.then((resolvedResult: any) => {
                          set((draft) => {
                            if (draft.subFlows[context.geoNodeId]?.nodeRuntime[target]) {
                              draft.subFlows[context.geoNodeId].nodeRuntime[target].output =
                                resolvedResult;
                            }
                          });
                        });
